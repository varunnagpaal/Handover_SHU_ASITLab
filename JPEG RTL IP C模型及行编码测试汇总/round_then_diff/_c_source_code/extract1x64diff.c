    ...
    for (y = 0; y < ysize; y += Vk)
    {
        ...
        for (i = 0; i < Vk; i++)
        {
            for (x = 0; x < xsize; x++)
            {
                ...
                tcb = cb;
                tcr = cr;
                l = l - linebefore[x * 3 + 0];
                cb = cb - linebefore[x * 3 + 1];
                cr = cr - linebefore[x * 3 + 2];
                if (y != 0)
                {
                    l += 127;
                    cb += 127;
                ...
                linebefore[x * 3 + 0] = tl;
                linebefore[x * 3 + 1] = tcb;
                linebefore[x * 3 + 2] = tcr;
            ...
            case 9:
                ...
                for (i = 0; i < 1; i++)
                    for (j = 0; j < 64; j++)
                    {
                        fputc(cbb[i][j], fpo);
                        fputc(cbb[i][j] >> 8, fpo);
                        fputc(cbb[i][j] >> 16, fpo);
                        fputc(cbb[i][j] >> 24, fpo);
                    }
                for (i = 0; i < 1; i++)
                    for (j = 0; j < 64; j++)
                    {
                        fputc(crb[i][j], fpo);
                        fputc(crb[i][j] >> 8, fpo);
                        fputc(crb[i][j] >> 16, fpo);
                        fputc(crb[i][j] >> 24, fpo);
                    }
                break;
            default:
                fprintf(stderr, "MCU format type not currently supported!!\n");
