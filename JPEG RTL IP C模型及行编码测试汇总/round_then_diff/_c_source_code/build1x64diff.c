		...
		for (x = 0; x < xsize; x += Hk)
		{
			...
			case 9:
				...
				for (i = 0; i < 1; i++)
					...
					{
						lb[i][j] = fgetc(fpi);
						lb[i][j] = lb[i][j] | (fgetc(fpi) << 8);
						lb[i][j] = lb[i][j] | (fgetc(fpi) << 16);
						lb[i][j] = lb[i][j] | (fgetc(fpi) << 24);
						lb[i][j] -= ifmin127;
				...
				for (i = 0; i < 1; i++)
					...
					{
						cbb[i][j] = fgetc(fpi);
						cbb[i][j] = cbb[i][j] | (fgetc(fpi) << 8);
						cbb[i][j] = cbb[i][j] | (fgetc(fpi) << 16);
						cbb[i][j] = cbb[i][j] | (fgetc(fpi) << 24);
						cbb[i][j] -= ifmin127;
				...
				for (i = 0; i < 1; i++)
					...
					{
						crb[i][j] = fgetc(fpi);
						crb[i][j] = crb[i][j] | (fgetc(fpi) << 8);
						crb[i][j] = crb[i][j] | (fgetc(fpi) << 16);
						crb[i][j] = crb[i][j] | (fgetc(fpi) << 24);
						crb[i][j] -= ifmin127;
