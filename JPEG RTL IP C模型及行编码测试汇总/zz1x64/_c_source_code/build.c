 ...
 /* Get the X and Y size */
  ...
  if (mcu_type == 1){
    ...
    exit(1);
                                ...
                                /* this time verification for multiple of Vk and Hk and added the resu                                              ltent value to the temp1 and temp2  */
  if((xsize+temp1) & (Hk-1)){
  ...
  for(y=0;y<ysize;y+=Vk){
	 ...
	 for(j=0;j<Hk;j++){
	   ...
	   if (col_conv){
          ...
          if(y==((ysize+temp3)-Vk)){   // this expression for last vk no of lines 

            if(i<(Vk-temp3)){          // this expression for missing no of lines 

              if(x==((xsize+temp1)-Hk)){ // this expression for last Hk no of bytes

                if(j<(Hk-temp1)){         // this expression for missing no of bytes
             *wp++ = (unsigned char) r;
	     *wp++ = (unsigned char) g;
	     *wp++ = (unsigned char) b;
             }
