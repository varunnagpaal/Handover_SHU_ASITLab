...
void main(int argc, char **argv)
{
	...
	int r, g, b, l, cb, cr;
	int lb[32][128], cbb[32][128], crb[32][128];
	int Hk, Vk;
	...
	if (argc < 6)
	{
		...
		fprintf(stderr, "              9 : By Line, 1x64 per2 diff 1:1:1\n");
		fprintf(stderr, "              10 : By Line, 1x128 per2diff 2:1:1\n");
		exit(1);
	}
			...
			case 9:
				for (i = 0; i < 1; i++)
					for (j = 0; j < 64; j++)
					{
						...
						if (linecnt % 2 != 0)
						{
							lb[i][j] = ((lb[i][j] - 128) << 1) + linebefore[x + j][0];
				...
				for (i = 0; i < 1; i++)
					for (j = 0; j < 64; j++)
					{
						...
						if (linecnt % 2 != 0)
						{
							cbb[i][j] = ((cbb[i][j] - 128) << 1) + linebefore[x + j][1];
				...
				for (i = 0; i < 1; i++)
					for (j = 0; j < 64; j++)
					{
						...
						if (linecnt % 2 != 0)
						{
							crb[i][j] = ((crb[i][j] - 128) << 1) + linebefore[x + j][2];
