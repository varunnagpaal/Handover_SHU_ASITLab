function csvdata = eva(name)
    ...
    ref1 = imread([name, '\', name, '_gimp60.ppm']);
    ref2 = imread([name, '\', name, '_gimp70.ppm']);
    ref3 = imread([name, '\', name, '_gimp80.ppm']);
    ref4 = imread([name, '\', name, '_gimp90.ppm']);
    ref5 = imread([name, '\', name, '_gimp91.ppm']);
    ref6 = imread([name, '\', name, '_gimp92.ppm']);
    ref7 = imread([name, '\', name, '_gimp93.ppm']);
    ref8 = imread([name, '\', name, '_gimp94.ppm']);
    ref9 = imread([name, '\', name, '_gimp95.ppm']);
    ref10 = imread([name, '\', name, '_gimp96.ppm']);
    ref11 = imread([name, '\', name, '_gimp97.ppm']);
    ref12 = imread([name, '\', name, '_gimp98.ppm']);
    ref13 = imread([name, '\', name, '_gimp99.ppm']);
    ...
    [ssimval(end+1), ~] = ssim(A, ref13);
    [peaksnr(end+1), snrval(end+1)] = psnr(A, ref13);
    % [peaksnr' ssimval'];
    ...
    clear filename
    filename(1, :) = [name, '/', name, '_gimp60.ppm '];
    filename(end + 1, :) = [name, '/', name, '_gimp70.ppm '];
    filename(end + 1, :) = [name, '/', name, '_gimp80.ppm '];
