...
main(int argc, char **argv)
{
    ...
    int *c0, *c1, *c2;
    int sorder[8][8] = {0, 1, 2, 3, 4, 5, 6, 7,
    ...
    if (argc < 4)
    {
        ...
        fprintf(stderr, "              10 : By Line, s1x64 1:1:1\n");
        exit(1);
    }
    ...
    else if (mcu_type == 7 || mcu_type == 8 || mcu_type == 10)
    {
        Hk = 64;
        Vk = 1;
    }
    ...
    for (y = 0; y < ysize; y += Vk)
    {
        ...
        for (x = 0; x < xsize; x += Hk)
        {
            ...
            case 10:
                for (i = 0; i < 1; i++)
                    for (j = 0; j < 64; j++)
                        fputc(lb[i][sorder[i][j]], fpo);

                for (i = 0; i < 1; i++)
                    for (j = 0; j < 64; j++)
                        fputc(cbb[i][sorder[i][j]], fpo);

                for (i = 0; i < 1; i++)
                    for (j = 0; j < 64; j++)
                        fputc(crb[i][sorder[i][j]], fpo);
                break;
            default:
                fprintf(stderr, "MCU format type not currently supported!!\n");
                exit(1);
