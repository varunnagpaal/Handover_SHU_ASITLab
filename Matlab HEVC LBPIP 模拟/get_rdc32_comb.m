function [img_rebuild, split_frame, mode_frame, rdc] = get_rdc32_comb(x, y, img_src, img_rebuild, split_frame, mode_frame, rdc_deep_layer, rdc_ind)
    PU = 32; %4 8 16 32 64

    % loop 部分
    [prederr_blk_loop, pred_blk_loop, mode_blk_loop] = mode_select_loop(img_src, img_rebuild, x, y, PU);
    % img_rebuild_temp_loop = prederr_blk_loop + pred_blk_loop;
    mode_frame_temp_loop = fill_blk(mode_frame, x, y, PU, mode_blk_loop);
    mode_bits_loop = cal_loop_mode_bits(mode_blk_loop, mode_frame_temp_loop, x, y);
